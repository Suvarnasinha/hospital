  ...
  try {
    ...
    if (allMedications) {
      allMedications.forEach(async (medicationdetail) => {
        // Check if the medication is one-time and scheduled for today
        if (medicationdetail.type === "one-time" && medicationdetail.date === dateWithoutTime) {
        ...
        // Check if the medication is recurring and scheduled for today
